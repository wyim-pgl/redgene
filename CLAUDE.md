...
- **The `canonical_v1` `RB-TDNA` / `LB-TDNA` entries in the element DBs are WRONG.** `TGAGCGTCGCAAAGGCGCTCGGTCT` / `GGCCTCGGCCTGAGAGCCAAAACAC` lack the invariant `CAGGATATAT` border core and appear in no construct. Present in the tracked `element_db/gmo_combined_db_v2.fa`, `db/gmo_all_combined_db.fa`, `db/gmo_corn_combined_db.fa`. The border scan does not use them; curating them out is still open.
