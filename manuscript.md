...
**Step 9: Targeted insert assembly.** We reconstructed full-length insert sequences through a four-phase process combining soft-clip junction detection, transgene-positive site identification, candidate read extraction, and iterative multi-assembler gap filling. In Phase 1, we scanned the host BAM (step 7) for bidirectional clusters of soft-clipped reads to identify candidate insertion sites. Reads were grouped by clip direction (right-clipped for forward-strand reads, left-clipped for reverse-strand reads), and clusters within a configurable window were merged. Sites with both 5-prime and 3-prime clip clusters were paired as candidate insertion loci, providing junction coordinates and consensus clip sequences as assembly seeds. In Phase 1.5, we identified transgene-positive sites by extracting all clip sequences (20 to 80 bp) from each candidate site and searching them against a combined transgene database using BLAST+ blastn-short (Camacho et al., 2009) with word_size=7. The transgene database comprised 131 EUginius GMO elements and 5,288 NCBI UniVec vector sequences with adapters and primers removed. Sites where at least one clip sequence matched a known transgene element or vector backbone at 80% or greater identity over 20 bp or longer alignment were classified as transgene-positive and retained as assembly targets; sites with no transgene hit were skipped as likely endogenous structural variants. The blastn-short task was required because standard megablast (word_size=28) cannot seed alignments on 20 to 30 bp clip sequences. We evaluated host-mapping-based filtering approaches (minimap2, Bowtie2, and BLAST against the host reference) for distinguishing transgene insertions from endogenous structural variants, but rejected them because cultivar-specific presence/absence variants (PAVs) in non-reference cultivars produce false "foreign" classifications for genomic segments absent from the reference assembly. Transgene-positive identification avoids this problem because all transgenic constructs use known regulatory elements (CaMV 35S, nos, nptII, bar, hpt) or vector backbones (pBI, pCAMBIA, pUC) that are represented in the transgene database. In Phase 2, we extracted candidate reads from the host BAM in three categories: junction-flanking reads within 5 kb of each transgene-positive junction coordinate, unmapped reads (SAM flag 0x4), and mate-unmapped reads (SAM flag 0x8). In Phase 3, we iteratively assembled the insert using a four-assembler loop: (1) k-mer greedy extension, which extends seed sequences by finding overlapping k-mers in the candidate read pool; (2) minimap2 soft-clip extension, which aligns candidate reads to current contigs and incorporates soft-clipped overhangs to extend contig ends; (3) Pilon v1.24 (Walker et al., 2014) gap filling with `--fix all --mindepth 3 --gapmargin 100000` and k-mer size 47; and (4) SSAKE overlap-layout-consensus assembly of remaining unplaced reads. The four-assembler cycle repeated until all four methods showed zero growth in a single round, indicating convergence, with a maximum of 15 rounds. In Phase 4, we annotated assembled inserts by local BLAST against a 278-sequence element database comprising 131 EUginius GMO elements, 82 CRL-GMOMETHODS amplicon sequences (Bonfini et al., 2012), 62 corn event-specific LB/RB border sequences (Ji et al., 2025), and 3 custom entries, followed by remote NCBI nt BLAST for segments not matched locally. T-DNA border motifs were identified by short-word BLAST (word size 7) against the canonical 25 bp border consensus.
...
Step 9 (targeted insert assembly) was applied to eight samples across five species (rice, tomato, cucumber, corn, and soybean) to reconstruct full-length insert sequences. Phase 1 soft-clip junction detection identified candidate insertion sites from the host BAM by clustering bidirectional soft-clipped reads. Phase 1.5 classified each candidate site as transgene-positive or endogenous by searching clip sequences (20 to 80 bp) against a combined transgene database (131 EUginius GMO elements and 5,288 NCBI UniVec vector sequences, adapters and primers removed) using blastn-short (word_size=7). Sites where at least one clip matched a known transgene element at 80% or greater identity over 20 bp or longer alignment were retained as assembly targets; sites with no transgene hit were skipped as likely structural variants. Phase 2 extracted candidate reads from junction regions and unmapped pairs. Phase 3 iteratively assembled inserts using a four-assembler loop (k-mer greedy extension, minimap2 soft-clip extension, Pilon gap fill, and SSAKE overlap-layout-consensus), converging when all four assemblers showed zero growth. Phase 4 annotated assembled inserts by local BLAST against the 278-sequence element database (131 EUginius elements, 82 CRL-GMOMETHODS amplicon sequences, 62 corn border sequences, and 3 custom entries) followed by remote NCBI nt BLAST for unmatched segments. Assembly and annotation results for these samples are pending completion.
